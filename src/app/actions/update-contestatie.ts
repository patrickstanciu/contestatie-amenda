...
export async function updateContestatieText(id: string, text: string): Promise<void> {
  ...
  await prisma.contestatie.update({
    where: { id },
    data: { textGenerat: text.trim() },
  });

  revalidatePath(`/contestatie/${id}`);
}
